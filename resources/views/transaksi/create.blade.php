				...
				<div class="row">
					<div class="col-lg-12">
						<div class="d-flex flex-column h-100">
							<p class="mb-1 pt-2 text-bold">Tambah Data Transaksi</p>
							<p class="mb-2"></p>
							<form action="" method="post">
								...
								<div class="form-group">
									...
									<select name="id_pesanan" class="form-control select2" required>
										...
										@if ($v->status != 'lunas')
										<option value="{{$v->id}}">{{$v->kd_pesanan.' - '.$v->penyewa.' - Kurang bayar '.rupiah($v->total_biaya - $v->dibayarkan)}}</option>
										@endif
										@endforeach
									</select>
									@error('id_pesanan')
								...
								<div class="form-group">
									...
									<div class="form-check form-check-inline">
										<label class="form-check-label">
											<input class="form-check-input status" type="radio" name="status" id="status_1" value="0" {{old('status')==0?'checked':null}}> Belum
										</label>
									</div>
									<div class="form-check form-check-inline">
										<label class="form-check-label">
											<input class="form-check-input status" type="radio" name="status" id="status_2" value="1" {{old('status')==1?'checked':null}}> Lunas
										</label>
									</div>
									<div class="form-check form-check-inline">
										<label class="form-check-label">
											<input class="form-check-input status" type="radio" name="status" id="status_3" value="3" {{old('status')==2?'checked':null}}> Gagal
										</label>
									</div>
									@error('status')
									<span class="error">{{$message}}</span>

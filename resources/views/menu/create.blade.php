							...
							<form action="" method="post">
								...
								<div class="form-group">
									...
									<select name="icon_id" class="form-control select2" required>
										...
										@foreach ($icon as $v)
										<option kode="{{$v->icon}}" {{old('icon_id')==$v->id?'selected':null}} value="{{$v->id}}">{{$v->icon}}</option>
										@endforeach
									</select>
									@error('icon_id')
									<span class="error">{{$message}}</span>
								...
								<div class="form-group">
									...
									<div class="form-check form-check-inline">
										<label class="form-check-label">
											<input class="form-check-input is_link" type="radio" name="is_link" id="is_link_1" value="1" {{old('is_link')?'checked':null}}> Yes
										</label>
									</div>
									<div class="form-check form-check-inline">
										<label class="form-check-label">
											<input class="form-check-input is_link" type="radio" name="is_link" id="is_link_2" value="0" {{!old('is_link')?'checked':null}}> No
										</label>
									</div>
									@error('is_link')
									<span class="error">{{$message}}</span>
								...
								<div class="form-group">
									...
									<div class="form-check form-check-inline">
										<label class="form-check-label">
											<input class="form-check-input is_separator" type="radio" name="is_separator" id="is_separator_1" value="1" {{old('is_separator')?'checked':null}}> Yes
										</label>
									</div>
									<div class="form-check form-check-inline">
										<label class="form-check-label">
											<input class="form-check-input is_separator" type="radio" name="is_separator" id="is_separator_2" value="0" {{!old('is_separator')?'checked':null}}> No
										</label>
									</div>
									@error('is_separator')
									<span class="error">{{$message}}</span>
	...
	function separator() {
		...
		if (is_separator== '1') {
			$("#for_separator").html(`<div class="form-group">
					...
					<input type="text" class="form-control" name="separator" id="separator" value="{{old('separator')}}" required placeholder="Separator..">
					@error('separator')
					<span class="error">{{$message}}</span>
					@enderror
				</div>`);
